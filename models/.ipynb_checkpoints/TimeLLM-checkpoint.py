...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        if configs.llm_model == 'LLAMA':
            ...
            # self.gpt2_config = GPT2Config.from_pretrained('openai-community/gpt2') # <--- 이거 주석 처리
            # [!!! 수정 !!!] 우리가 configs.llm_model_path에 넣어둔 '로컬 경로'를 쓰도록 변경
            ...
            try:
                self.llm_model = GPT2Model.from_pretrained(
                    # 'openai-community/gpt2',  <--- 이거 지우고
                    model_path,               # <--- [!!! 수정 !!!] model_path 사용
                    trust_remote_code=True,
                    local_files_only=True,
                    config=self.gpt2_config,
                )
            except EnvironmentError: 
                # ... (다운로드 로직은 이제 필요 없으니 무시하거나, 똑같이 model_path로 수정)
                print("Local model files not found. Attempting to download...")
                self.llm_model = GPT2Model.from_pretrained(
